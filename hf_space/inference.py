"""Carrega artefatos e prediz Qualidade_Ambiental."""
...
def validate_inputs(values: tuple[float | None, ...]) -> tuple[list[str], list[str]]:
    """
    Valida os inputs seguindo faixas realistas.

    Retorna (errors, warnings). Em caso de errors, a UI deve bloquear a predição.
    """
    errors: list[str] = []
    warnings: list[str] = []
    ...
    for name, value in zip(FEATURE_COLUMNS, values, strict=True):
        ...
        if hard_min is not None and v < float(hard_min):
            hard_violation = True
        if hard_max is not None and v > float(hard_max):
            hard_violation = True
        ...
        if soft_min is not None and v < float(soft_min):
            soft_violation = True
        if soft_max is not None and v > float(soft_max):
            soft_violation = True
...
def _load_artifacts() -> None:
    ...
    if not model_path.is_file() or not le_path.is_file():
        raise FileNotFoundError(
            ...
            "Execute: python scripts/export_hf_artifacts.py"
        )
...
def predict_qualidade(row: dict[str, float]) -> tuple[str, str]:
    """
    Retorna (classe prevista, texto auxiliar com probabilidades se existirem).
    """
    _load_artifacts()
...
def predict_from_tuple(values: tuple[float | None, ...]) -> tuple[str, str]:
    if len(values) != len(FEATURE_COLUMNS):
        raise ValueError(f"Esperado {len(FEATURE_COLUMNS)} valores.")
    if any(v is None for v in values):
        raise ValueError("Preencha todos os campos numéricos.")
    row = dict(zip(FEATURE_COLUMNS, values, strict=True))
    return predict_qualidade(row)
